...
public class Car implements I_CarCollisionObserver {
	...
	private double currentSpeed, angle;
	private Lane track;
	private Road road;
	private Point position;
	private Color color;
	private String imagePath;
	private I_CarState state;
	private BufferedImage carImage;
	
	public Car(){
		
	}
	public Car(Point xy, int cellId, Road road, int laneId, int numLanes, int id, String imagePath) {
		 this.position = xy;
	     ...
	     this.vehicleId = id;
		 this.state = new CarDrivingState();
	...
	public void collisionNotification(int vehicle1ID, int vehicle2ID) {
		if (this.vehicleId == vehicle1ID || this.vehicleId == vehicle2ID) {
		    this.state = new CarCrashedState();
        }
	}
    ...
    public boolean isCrashed() {
        return (this.state.getState() == CarStatesEnum.CRASHED);
    }
    ...
    public CarStatesEnum getStateEnum() {
        return this.state.getState();
    }
    
    public I_CarState getState() {
        return this.state;
    }
