...
public class FastDriver extends Driver {
	
	private static int speedModifier = 4;
	private static double crashChance = 0.0005;

    public FastDriver(Car _vehicle, Vision _sight) {
	...
	public static void weatherSpeedUpdate() {
		...
		if (WeatherState.getState() == "RAINY") {
			speedModifier = (int)Speed.FastSpeed(1);
			crashChance = 0.0005;
		}
		if (WeatherState.getState() == "SNOWY") {
			speedModifier = (int)Speed.FastSpeed(0);
			crashChance = 0.0007;
		}
	}
