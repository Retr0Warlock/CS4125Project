...
public class SlowDriver extends Driver {
	
	private static int speedModifier = 1;
	private static double crashChance = 0.0001;

    public SlowDriver(Car _vehicle, Vision _sight) {
	...
	public static void weatherSpeedUpdate() {
		...
		if (WeatherState.getState() == "RAINY") {
			speedModifier = (int)Speed.SlowSpeed(1);
			crashChance = 0.0002;
		}
		if (WeatherState.getState() == "SNOWY") {
			speedModifier = (int)Speed.SlowSpeed(0);
			crashChance = 0.0005;
		}
	}
